...
public class ScanIn {
        ...
        public Employee[] creatEmployees(String schedule) {
            ...
            for (int x = 0; x < employees.length; x++) {
                name = employeeStrings[x].substring(0, employeeStrings[x].indexOf(","));
                employeeStrings[x] = employeeStrings[x].substring(employeeStrings[x].indexOf(",") + 1);
                if (firstCharSpace(employeeStrings[x])) {
                    position = employeeStrings[x].substring(1, employeeStrings[x].indexOf(","));
                }
                else {
                    employeeStrings[x].substring(0, employeeStrings[x].indexOf(","));
                }
                employeeStrings[x] = employeeStrings[x].substring(employeeStrings[x].indexOf(",") + 1);
                if (firstCharSpace(employeeStrings[x])) {
                    shiftStart = Integer.parseInt(employeeStrings[x].substring(1, employeeStrings[x].indexOf("-")));
                }
                else {
                    shiftStart = Integer.parseInt(employeeStrings[x].substring(0, employeeStrings[x].indexOf("-")));
                }
                shiftLength = Integer.parseInt(employeeStrings[x].substring(employeeStrings[x].indexOf("-")+1)) - shiftStart;
